...
import type { Business, ViewState, Coordinates } from '@/types';
...
// El mapa solo se carga en el cliente (MapLibre necesita DOM/WebGL)
...
export const Route = createFileRoute('/')({
  validateSearch: (search: Record<string, unknown>): SearchParams => ({
    view:
      search.view === 'ADD_BUSINESS' || search.view === 'DETAILS'
        ? search.view
        : undefined,
    ...
    business:
      typeof search.business === 'string' ? search.business : undefined,
  }),
  component: HomePage,
});
...
function HomePage() {
  ...
  // URL-based state
  const viewState: ViewState = search.view || 'LIST';
  const searchQuery = search.q || '';
  const selectedDistrict = search.district || '';
  const selectedCategory = search.category || '';
  const activeBusinessId = search.business || null;
  ...
  const handleSaveBusiness = async (newBusiness: Business) => {
    try {
      ...
      setMapCenter({
        lat: Number(createdBusiness.lat),
        lng: Number(createdBusiness.lng),
      });
      updateURL({
      ...
      alert(
        'No se pudo guardar el negocio. Verifica que hayas iniciado sesión con Google.',
      );
    }
  };

  const handleSearchChange = (value: string) => {
    // replace: true evita llenar el historial con cada tecla
    updateURL(
      {
        q: value || null,
        district: selectedDistrict || null,
    ...
    <div className="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-gray-100 font-sans text-gray-900">
      ...
      <div className="relative w-full h-full md:w-[65%] lg:w-[70%] z-0 bg-[#262626]">
        {mounted ? (
          <Suspense
            fallback={
              <div className="w-full h-full bg-[#262626] flex items-center justify-center text-gray-500">
                Cargando mapa de Lima...
              </div>
            }
          >
            <MapBoard
        ...
        <div className="flex-1 overflow-hidden relative">
          {viewState === 'ADD_BUSINESS' ? (
            ...
            <div className="h-full flex flex-col">
              ...
              {/* ACTION BAR */}
              <AuthActionBar onAddBusiness={handleAddBusiness} />
                    ...
                    <div className="flex-1">
                      {filteredBusinesses.map((b) => (
                        <BusinessCard
                          ...
                          isActive={String(activeBusinessId) === String(b.id)}
                        />
                      ))}
                    </div>
                  </div>
...
function AuthActionBar({ onAddBusiness }: { onAddBusiness: () => void }) {
