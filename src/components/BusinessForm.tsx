...
export const BusinessForm: React.FC<BusinessFormProps> = ({ onCancel, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Normaliza website a http(s); rechaza esquemas peligrosos (javascript:, etc.)
    const website = safeHttpUrl(formData.website);
    if (!website) {
      setError("El sitio web no es una URL válida (solo http/https).");
      return;
    }
    ...
    const newBusiness: Business = {
      ...
      imageUrl: `https://picsum.photos/seed/${encodeURIComponent(formData.name || 'negocio')}/400/300`
    };
